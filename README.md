...
A saída no console é só o formato oficial exigido pelo desafio — nada mais. As informações de memória, que não fazem parte desse formato, são gravadas num arquivo de log (`memoria.log`, criado na pasta de onde o comando é executado) em vez de aparecer no terminal, para nunca contaminar visualmente nem colocar em risco uma validação automática da saída.
...
- O diagnóstico de memória por arquivo fica em `memoria.log` (ignorado pelo git via `.gitignore`), não no console — assim a saída no terminal é sempre exatamente o formato exigido, sem precisar de nenhum redirecionamento manual.
